...
function getfestivalPage(){
  ...
  .then(function(response){
    ...
    iframe.setAttribute("src",`https://www.google.com/maps/embed/v1/place?key=${config.apiKey}&q=${thisData.Location}&zoom=17&center=${thisData.Position.PositionLat},${thisData.Position.PositionLon}`);    
    ...
    nearbyTourism.setAttribute("href",`./nearby-tourism.html?theme=節慶活動=${thisData.ActivityID}`);
    nearbyFestival.setAttribute("href",`./nearby-festival.html?theme=節慶活動=${thisData.ActivityID}`);
    nearbyFood.setAttribute("href",`./nearby-food.html?theme=節慶活動=${thisData.ActivityID}`);
...
function getTourPageMore(){
  axios.get(`https://ptx.transportdata.tw/MOTC/v2/Tourism/Activity/${city}?%24format=JSON`, {
    headers: getAuthorizationHeader()
  })
  .then(function(response){
    // 搖亂thisDate陣列
